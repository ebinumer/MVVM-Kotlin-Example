package com.eduvy.demo.data.repo.helpers
...
import com.eduvy.demo.data.repo.helpers.DataFetchHelper.DataFetchStyle.*
...
abstract class DataFetchHelper<T>(
    ...
    abstract class NetworkCall<S>(
        tag: String
    ) : DataFetchHelper<S>(tag, NETWORK_ONLY_WITHOUT_LOCAL) {
        abstract override suspend fun getDataFromNetwork(): Response<out Any?>
        abstract override suspend fun convertApiResponseToData(response: Response<out Any?>): S
    }
    ...
    open suspend fun convertApiResponseToData(response: Response<out Any?>): T {
        Timber.e("convertApiResponseToData=${Gson().toJson(response.body())}")
        try {
    ...
    private suspend fun fetchDataByStyle(
        ...
        when (dataFetchStyle) {
            ...
            NETWORK_ONLY_WITHOUT_LOCAL -> {
                resource.data = refreshDataFromNetwork(resource, NETWORK_ONLY_WITHOUT_LOCAL)
                resource.fresh = true
                resource.dataFetchStyleResult = Result.NETWORK_DATA_ONLY
            }
        }
    ...
    private suspend fun refreshDataFromNetwork(
        ...
        try {
            ...
            Timber.e("Called body=${response.raw().request.body}")
            Timber.e("Called token=${response.raw().request.header("Authorization")}")
